...
# Path to dataset
data_dir = "/Users/ruthv/Downloads/WaterPollution/waterpoldataset"
...
def visualize_prediction_with_cam(model, image_path, class_names):
    """
...
def get_model(train=False):
    # Load datasets if needed for training
    if train:
        image_datasets = {
        ...
        evaluate_model(model, dataloaders['test'], image_datasets['train'].classes)
    else:
        # Just create the model and load weights
        model = WaterPollutionCNN().to(device)
...
def predict_test_image(model, image_name):
    """
    ...
    # Get class names
    try:
        test_dataset = datasets.ImageFolder(f'{data_dir}/test', transform=data_transforms)
        class_names = test_dataset.classes
    except:
        # Fallback if dataset can't be loaded
        class_names = ['natural', 'trash']
    ...
    for class_folder in class_names:
        ...
        print(f"Trying image path: {image_path}")
        ...
        if os.path.exists(image_path):
            ...
            # Visualize with CAM
            prediction, confidence = visualize_prediction_with_cam(model, image_path, class_names)
            return prediction
...
def main():
    parser = argparse.ArgumentParser(description='Water Pollution CNN with CAM')
    parser.add_argument('--train', action='store_true', help='Train the model')
    parser.add_argument('--predict', type=str, help='Predict on a test image and show CAM')
    
    args = parser.parse_args()
    
    # Load or train the model
    model = get_model(train=args.train)
    
    # Predict on a test image if specified
    if args.predict:
        predict_test_image(model, args.predict)
    elif not args.train:
        # If no command line args provided, predict on a default image
        print("No image specified, using default image24.jpg")
        predict_test_image(model, "image2.jpg")
